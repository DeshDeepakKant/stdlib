...
#### stdlib_base_dists_kumaraswamy_pdf( x, alpha, beta )

Evaluates the [probability density function][pdf] (PDF) of a [Kumaraswamy][kumaraswamy-distribution] distribution with parameters `alpha` (first shape parameter) and `beta` (second shape parameter).

```c
double y = stdlib_base_dists_kumaraswamy_pdf( 0.5, 2.0, 1.0 );
// returns 1.0
```
...
```c
double stdlib_base_dists_kumaraswamy_pdf( const double x, const double alpha, const double beta );
```
...
int main( void ) {
    ...
    for ( i = 0; i < 25; i++ ) {
        ...
        y = stdlib_base_dists_kumaraswamy_pdf( x, alpha, beta );
